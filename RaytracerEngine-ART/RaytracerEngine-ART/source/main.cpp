...
int main() {
	...
	Scene scene = loadScene(read("mobius.obj"));
	RenderMesh monkey = scene.genRenderMesh(scene.indexByName("Cylinder"));
	RenderMesh shadow = scene.genRenderMesh(scene.indexByName("Cylinder"));
	RenderMesh floor = scene.genRenderMesh(scene.indexByName("Cube"));

	Renderer camera;
	if (!camera.init(width * 2, height * 2, width, height, 12500, 12500)) {
		return -1;
	}
	...
	do {
		float radius = 10;
		vec3 cameraPos = vec3(12.5, 2.5, -12.5);
		mat4 mvp = perspective(radians(25.0f), float(width)/height, 0.1f, 100.0f);
		mvp = mvp * lookAt(cameraPos, vec3(0), vec3(0, 1, 0));

		camera.beginDraws(cameraPos, vec3(5, 6.25, 3));
		camera.drawObjectToBuffers(obj, mvp, sky);
		camera.drawObjectToBuffers(floor_mesh, mvp, sky);
		camera.shade();
